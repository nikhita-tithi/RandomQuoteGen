...
const myFunction = () => {
  fetch(
    ...
    .then((item) => {
      const quoarr = item.quotes;

      const randbgcol = Math.floor(Math.random() * bgcolors.length + 1);

      const randQuoteno = Math.floor(Math.random() * quoarr.length);
      const randAuthor = quoarr[randQuoteno].author;
